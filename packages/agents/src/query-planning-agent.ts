import type { QueryPlanningAgent as IQueryPlanningAgent, AgentResult } from "./interfaces"

export class QueryPlanningAgent implements IQueryPlanningAgent {
  async process(payload: any): Promise<AgentResult> {
    try {
      const result = await this.plan(payload.question)
      return {
        success: true,
        data: result,
      }
